...
#else
        #define VK_DEFINE_NON_DISPATCHABLE_HANDLE(object) typedef uint32_t object;
#endif
#endif
...
typedef VkResult (VKAPI_PTR *PFN_vkWaitForFences)(VkDevice device, uint32_t fenceCount, const VkFence* pFences, VkBool32 waitAll, uint32_t timeout);
...
typedef VkResult (VKAPI_PTR *PFN_vkAcquireNextImageKHR)(VkDevice device, VkSwapchainKHR swapchain, uint32_t timeout, VkSemaphore semaphore, VkFence fence, uint32_t* pImageIndex);
...
typedef struct VkWin32KeyedMutexAcquireReleaseInfoKHR {
    ...
    const uint32_t*          pAcquireKeys;
    const uint32_t*          pAcquireTimeouts;
    uint32_t                 releaseCount;
    const VkDeviceMemory*    pReleaseSyncs;
    const uint32_t*          pReleaseKeys;
...
typedef struct VkD3D12FenceSubmitInfoKHR {
    ...
    const uint32_t*    pWaitSemaphoreValues;
    uint32_t           signalSemaphoreValuesCount;
    const uint32_t*    pSignalSemaphoreValues;
...
typedef void (VKAPI_PTR *PFN_vkDebugReportMessageEXT)(VkInstance instance, VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint32_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage);
...
typedef VkResult (VKAPI_PTR *PFN_vkGetSwapchainCounterEXT)(VkDevice device, VkSwapchainKHR swapchain, VkSurfaceCounterFlagBitsEXT counter, uint32_t* pCounterValue);
...
typedef struct VkRefreshCycleDurationGOOGLE {
    uint32_t    refreshDuration;
} VkRefreshCycleDurationGOOGLE;

typedef struct VkPastPresentationTimingGOOGLE {
    uint32_t    presentID;
    uint32_t    desiredPresentTime;
    uint32_t    actualPresentTime;
    uint32_t    earliestPresentTime;
    uint32_t    presentMargin;
